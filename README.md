...
Place this folder directly inside the initialized Moonshine runtime home, not inside the Moonshine source-code package. With the default Moonshine setup, the runtime home is `~/.moonshine`. If Moonshine was initialized with `--home`, use that directory instead.
...
Create a UTF-8 JSON file containing an ordered list of objects:
...
`materials` is optional and accepts local UTF-8 text or Markdown files. Relative paths are resolved from the directory containing the input JSON file.

## Run

Run commands from the initialized Moonshine runtime home. For a default installation:

```bash
cd ~/.moonshine
```

For a custom runtime home, change into the same directory previously passed to `python -m moonshine --home <path> init`.

Validate the input without starting Moonshine sessions:
